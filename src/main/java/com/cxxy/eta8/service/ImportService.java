...
public class ImportService {
    ...
    public boolean importAward(File xls) throws Exception {
        FileInputStream fis = new FileInputStream(xls);
        HSSFWorkbook workbook = new HSSFWorkbook(fis); //HSSF － 提供读写Microsoft Excel格式档案的功能。
        HSSFSheet sheet = workbook.getSheetAt(0);

        final List<Record> records = new ArrayList<Record>(sheet.getLastRowNum());

        for (int i = 6; i <= sheet.getLastRowNum(); i++) {
            HSSFRow row = sheet.getRow(i);
            String awardName = row.getCell(0).getStringCellValue();
            Integer awardTypeId = (int) row.getCell(1).getNumericCellValue();
            if (StrKit.isBlank(awardName) || StrKit.isBlank(awardTypeId.toString())) {
                /*
            ...
            // 如果导入的记录在Award表中不存在的话，才导入该行数据
            List<Record> aRecords = new DbRecord(DbConfig.T_AWARD)
                    .whereEqualTo("awardName", awardName)
                    .whereEqualTo("awardTypeId", awardTypeId)
                    .query();
            if (aRecords.isEmpty()) {
                Record record = new Record();
                record.set("awardName", awardName);
                record.set("awardTypeId", awardTypeId);
                records.add(record);
            }
        }
        workbook.close();
        ...
        return Db.tx(new IAtom() {
            public boolean run() throws SQLException {
                Db.batchSave(DbConfig.T_AWARD, records, records.size());
                return true;
            }
        });
    }
